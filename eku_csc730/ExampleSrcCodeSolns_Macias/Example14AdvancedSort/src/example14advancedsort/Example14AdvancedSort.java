...
class MyArray
{
    ...
    public void mergeSortNonrecursive()
	{
		...
		for (int len = 2; len < 2 * n; len = len*2) // len: length of the subarray to be sorted (2, 4, 8, ...)
		{
			for (int left = 0; left < n; left = left + len)
			{
				int right = (left + len - 1 < n)? left + len - 1 : (n-1);
				// System.out.println("left=" + left + " right=" + right);
				merge(left, left + (len - 1) / 2, right);
			}
		}
	}
}